...
class StereoCalibrator(Calibrator):
    ...
    def cal_fromcorners(self, good):
        ...
        if self.camera_model == CAMERA_MODEL.PINHOLE:
            ...
            print(f"stereo reprojection error: {ret_values[0]}")
    ...
    def chessboard_size(self, lcorners, rcorners, board, msg=None):
        ...
        pt3d = [cam.projectPixelTo3d(
            (lcorners[i, 0, 0], lcorners[i, 0, 1]), disparities[i, 0]) for i in range(lcorners.shape[0])]
    ...
    def handle_msg(self, msg):
        ...
        if self.calibrated:
            ...
            if lcorners is not None and rcorners is not None and len(lcorners) == len(rcorners):
                # Add samples with entire board in view if charuco
